...
static uint8_t timeCheckDstStatus(const time_complex_t * xInTime);
//-----------------------------------------------------------------------------
...
uint8_t timeMaxDayOfMonthGet(uint8_t xMonth, uint16_t xYear)
{
	...
	if (2099 < xYear)
	{
		UART_PutString(SC"e: timeMaxDayOfMonthGet - zbyt duza wartosc pola rok - dopuszczalne 0-99\r\n");
	}
	...
	{
		UART_PutString(SC"e: timeMaxDayGet - nieprawidlowy nr miesiaca\r\n");
	}
...
uint8_t timeUtcToLocalConv(const time_complex_t * xInUtcTime, time_complex_t * xOutLocalTime)
{
	...
	if ((!localtimeZoneErr)&& (!dstFlagErr))
	{
		...
		{
			UART_PutString(SC"i: timeUtcToLocalConv - przekroczony rok 2036 lub ponizej 2012, timestamp zawiera bledne dane\r\n");
			err	= TRUE;
		}
	}
	else
	{
		UART_PutString(SC"e: timeUtcToLocalConv -  nieprawidlowy parametr\r\n");
		err	= TRUE;
	}
...
uint8_t timeLocalToUtcConv(const time_complex_t * xInLocalTime, time_complex_t * xOutUtcTime)
{
	...
	{
		UART_PutString(SC"i: timeLocalToUtcConv - przekroczony rok 2036 lub ponizej 2012, timestamp zawiera bledne dane\r\n");
		err	= TRUE;
	}
